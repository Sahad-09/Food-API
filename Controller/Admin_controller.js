...
const UpdateFood = async (req, res) => {
    try {
        ...
        if (!data) {
            ...
            const { itemName: itemName, price: price, description: description } = req.body
            let newData = {}
            if (itemName) { newData.itemName = itemName }
            if (price) { newData.price = price }
            if (description) { newData.description = description }

            const UpdatedData = await MenuSchema.findByIdAndUpdate(id, { $set: newData }, { new: true })
            return res.json({ success: true, UpdatedData: UpdatedData })
...
module.exports = { Register, View, Delete, Update, Login, AddFood, DeleteFood, ViewFood, UpdateFood }
